      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
          <div>
            <div className="flex items-center space-x-2">
              <h1 className="text-2xl font-bold text-white tracking-tight">Regulatory Source & Model Registry</h1>
              ...
              {sources.map((src) => (
                <div key={src.source_id} className="glass-panel rounded-xl p-5 border-slate-800 flex flex-col justify-between">
                  <div>
                    ...
                    <h3 className="text-sm font-bold text-white mb-2 leading-snug">{src.title}</h3>
                    <p className="text-xs text-slate-400 leading-relaxed mb-4">{src.notes}</p>
                  </div>
        ...
        {activeTab === 'methodologies' && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {methodologies.map((m) => (
              <div key={m.code} className="glass-panel rounded-xl p-4 border-slate-800 text-xs flex flex-col justify-between">
                <div>
        ...
        {activeTab === 'models' && (
          <div className="space-y-4">
            {[
              ...
              <div key={m.id} className="glass-panel rounded-xl p-5 border-slate-800">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-3 mb-3 border-b border-slate-800 gap-2">
                  <div className="flex items-center space-x-2">
                    <span className="font-mono text-sm font-bold text-emerald-400">{m.id}</span>
                    <span className="text-sm font-bold text-white">· {m.name}</span>
                  </div>
                  <span className="px-2.5 py-0.5 rounded text-[11px] font-mono font-bold bg-emerald-950 text-emerald-300 border border-emerald-800 self-start sm:self-auto">
                    TIER: {m.tier}
                  </span>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                  <div className="space-y-2">
                    <div><span className="text-slate-400 font-medium">Algorithm:</span> <span className="font-mono text-slate-200">{m.algo}</span></div>
                    <div><span className="text-slate-400 font-medium">Target:</span> <span className="font-mono text-white">{m.target}</span></div>
                    <div><span className="text-slate-400 font-medium">Features ({m.features.length}):</span> <span className="font-mono text-sky-300">{m.features.join(', ')}</span></div>
                  </div>
                  <div className="space-y-2">
                    <div><span className="text-slate-400 font-medium">Holdout MAE:</span> <span className="font-mono text-emerald-400 font-bold">{m.mae}</span></div>
                    <div><span className="text-slate-400 font-medium">Lift over Baseline:</span> <span className="font-mono text-slate-200">{m.lift}</span></div>
                    <div><span className="text-slate-400 font-medium">Governance:</span> <span className="text-slate-300">{m.notes}</span></div>
                  </div>
                </div>
              </div>
            ))}
        ...
        {activeTab === 'provenance' && (
          <div className="space-y-4">
            {[
              ...
              <div key={p.id} className="glass-panel rounded-xl p-5 border-slate-800 text-xs">
                <div className="flex justify-between items-center mb-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-slate-400 mb-2">
                  <div><strong className="text-slate-300">Authority:</strong> {p.authority}</div>
                  <div><strong className="text-slate-300">Record Count:</strong> {p.records}</div>
                </div>
                <div className="p-2.5 rounded-lg bg-slate-900 text-slate-300 font-mono text-[11px] leading-relaxed">
                  {p.source}
                </div>
              </div>
            ))}
