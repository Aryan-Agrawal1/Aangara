      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6">
        <div className={`flex items-center justify-between transition-all duration-200 ${scrolled ? "h-12" : "h-14"}`}>
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={toggle} className="w-11 h-11 flex items-center justify-center rounded-lg hover:bg-[#F6F8F7] text-[#4B5A54] hover:text-[#10231C] transition-colors border border-[#E4E9E6]" aria-label={`Switch to ${theme === "light" ? "dark" : "light"} mode`}>
              {theme === "light" ? <Moon className="w-5 h-5" /> : <Sun className="w-5 h-5" />}
            </button>
            <button className="lg:hidden w-11 h-11 flex items-center justify-center rounded-lg hover:bg-[#F6F8F7] text-[#4B5A54] border border-[#E4E9E6]" onClick={() => setMobileOpen(!mobileOpen)} aria-label="Toggle navigation" aria-expanded={mobileOpen}>
